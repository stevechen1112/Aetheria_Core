...
def test_mode_3_cross_validation():
    ...
    if response.status_code == 200:
        ...
        if result['status'] == 'success':
            ...
            # 紫微信息
            print(f"\n紫微命盘：")
            print(f"  命宫：{data['ziwei_chart'].get('locked_palace', '未知')}")
            print(f"  身宫：{data['ziwei_chart'].get('body_palace', '未知')}")
            print(f"  主星：{', '.join(data['ziwei_chart'].get('main_stars', []))}")
            ...
            with open(filename, 'w', encoding='utf-8') as f:
                ...
                f.write(f"身宫：{data['ziwei_chart'].get('body_palace', '未知')}\n")
